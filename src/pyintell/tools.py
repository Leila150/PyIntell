...
class Tool:
    ...
    def __post_init__(self):
        ...
        self.aliases = tuple(dict.fromkeys(
            str(x).strip() for x in self.aliases if str(x).strip()
        ))
        self.metadata = dict(self.metadata or {})

    def __call__(self, *args, **kwargs):
        if not self.enabled:
            raise ToolDisabledError(f"Tool '{self.name}' is disabled")
    ...
    @property
    def module(self):
        return getattr(self.function, "__module__", None)

    @property
    def qualified_name(self):
        return getattr(self.function, "__qualname__", self.name)
...
class ToolRegistry:
    ...
    def _key(value):
        ...
        if not key:
            raise TypeError("tool must be a function, Tool, or name")
    ...
    def add(self, function=None, name: Optional[str] = None, description: str = "",
            trusted: bool = False, replace: bool = False, aliases=(), enabled: bool = True,
            **metadata):
        def register(fn):
            if isinstance(fn, Tool):
                raise TypeError("use registry.add(function=...) with a callable, not a Tool instance")
            if not callable(fn):
            ...
            with self._lock:
                existing = tool_name in self._tools or tool_name in self._aliases
                if existing:
                    if not replace:
                        raise ValueError(f"Tool '{tool_name}' is already registered")
                    self.remove(tool_name)
                conflicts = [a for a in alias_values
                             if a == tool_name or a in self._tools or a in self._aliases]
                if conflicts:
                    raise ValueError(f"Tool alias is already registered: {conflicts[0]!r}")
                item = Tool(
                    fn, tool_name, description or inspect.getdoc(fn) or "",
                    bool(enabled), trusted, dict(metadata), alias_values
                )
    ...
    def remove(self, function):
        with self._lock:
            ...
            if item.enabled and item._on_disable:
                item._on_disable()
            return True
    ...
    def edit(self, function, contents=None, new_name=None, **changes):
        with self._lock:
            ...
            new_aliases = item.aliases if "aliases" not in changes else tuple(dict.fromkeys(
                str(x).strip() for x in changes["aliases"] if str(x).strip()
            ))
            ...
            new_item = Tool(
                ...
                bool(changes.get("enabled", item.enabled)),
                bool(changes.get("trusted", item.trusted)),
                new_metadata,
                new_aliases,
                item._on_enable,
                item._on_disable,
            ...
            if was_enabled and not new_item.enabled and new_item._on_disable:
                new_item._on_disable()
            elif not was_enabled and new_item.enabled and new_item._on_enable:
                new_item._on_enable()
    ...
    def get_tool(self, name):
        with self._lock:
            ...
            try:
                ...
                raise KeyError(f"Tool '{resolved}' is not registered") from None
    ...
    def enable(self, name):
        with self._lock:
            ...
            if not item.enabled:
                # Enable callback runs before exposing the enabled state. If it fails,
                # the tool remains disabled instead of becoming half-enabled.
                if item._on_enable:
                    item._on_enable()
                item.enabled = True
            return item
...
def _code_execution_impl(
    ...
    return executor.run(
        code, language, timeout=timeout, stdin=stdin, filename=filename,
...
_tool = tool.get_tool("code_execution")
_tool._on_enable = _enable_execution
_tool._on_disable = _disable_execution
tool.disable("code_execution")
code_execution = _tool
