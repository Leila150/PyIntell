...
class ExecutionResult:
    ...
    def as_dict(self):
        return {"language": self.language, "command": list(self.command), "returncode": self.returncode,
                "stdout": self.stdout, "stderr": self.stderr, "timed_out": self.timed_out,
                "duration": self.duration, "ok": self.ok, "runtime_available": self.runtime_available,
                "file": self.file, "metadata": dict(self.metadata)}
...
class ExecutionPolicy:
    def __init__(self, enabled=False, default_timeout=30, max_output=1_000_000,
        ...
        self.allowed_languages = ({str(x).lower() for x in allowed_languages} if allowed_languages else None)
        self.allow_network = bool(allow_network)

    def allow(self, language):
        if self.allowed_languages is None: return True
...
class CodeExecutor:
    ...
    def configure(self, **values):
        for key, value in values.items():
            if not hasattr(self.policy, key): raise ValueError(f"Unknown execution policy option: {key}")
    ...
    def runtime_info(self, language=None):
        ...
        return {"language": lang.name, "available": path is not None, "command": command,
                "path": path, "extensions": list(lang.extensions), "gui": lang.gui,
    ...
    def _commands(self, language, runtime, path, args=()):
        ...
        if name in {"Rust", "Fortran", "Pascal"}:
            artifact = str(path.with_suffix(".pyintell_bin"))
            return [runtime, str(path), "-o", artifact], [artifact, *args], artifact
        if name == "Go": return [runtime, "run", str(path), *args], None, None
        if name == "Java":
            if runtime != "javac": raise RuntimeUnavailableError("Java execution requires javac")
        ...
        if name == "Kotlin":
            if runtime != "kotlinc": raise RuntimeUnavailableError("Kotlin execution requires kotlinc")
            artifact = str(path.with_suffix(".jar"))
            return [runtime, str(path), "-include-runtime", "-d", artifact], ["java", "-jar", artifact, *args], artifact
        if name == "Zig": return [runtime, "run", str(path), *args], None, None
        if name == "GDScript": return [runtime, "--headless", "--script", str(path), *args], None, None
        if name == "Shell": return [runtime, str(path), *args], None, None
        if name == "PowerShell": return [runtime, "-File", str(path), *args], None, None
        if name == "WebAssembly": return [runtime, str(path), *args], None, None
    ...
    def run(self, code=None, language=None, *, filename=None, timeout=None, cwd=None,
        ...
        if not self.policy.enabled: raise ExecutionDisabledError("Code execution is disabled")
        if code is None and not filename: raise ValueError("provide code or filename")
        if filename is not None and code is not None: raise ValueError("provide either code or filename, not both")
        lang = get_language(language, filename) if language else (detect_language(filename) if filename else get_language("python"))
        if not self.policy.allow(lang): raise PermissionError(f"Language '{lang.name}' is not permitted")
        runtime, runtime_path = self._runtime(lang)
        if not runtime: raise RuntimeUnavailableError(f"No installed runtime/compiler found for {lang.name}: {lang.commands}")
        ...
        run_env = os.environ.copy(); run_env.update(self.policy.env)
        ...
        if total_timeout <= 0: raise ValueError("timeout must be greater than zero")
        started = time.monotonic(); artifact = None
        try:
            compile_command, run_command, artifact = self._commands(lang, runtime, path, args)
            if compile and run_command is not None:
                remaining = max(0.1, total_timeout - (time.monotonic() - started))
                rc, out, err, timed, _ = self._run_command(compile_command, cwd=run_cwd, env=run_env,
                                                            timeout=remaining, stdin=None)
                if timed or rc != 0:
                    return ExecutionResult(lang.name, compile_command, rc, out[:limit], err[:limit], timed,
                                           ...
                                           {"runtime": runtime, "runtime_path": runtime_path, "gui": lang.gui,
                                            "frameworks": list(lang.frameworks), "phase": "compile"})
                remaining = max(0.1, total_timeout - (time.monotonic() - started))
                rc, run_out, run_err, timed, _ = self._run_command(run_command, cwd=run_cwd, env=run_env,
                                                                    timeout=remaining, stdin=stdin)
                return ExecutionResult(lang.name, run_command, rc, (out + run_out)[:limit],
                                       ...
                                       {"runtime": runtime, "runtime_path": runtime_path, "gui": lang.gui,
                                        "frameworks": list(lang.frameworks), "phase": "run", "artifact": artifact})
...
def code_execute(code, language="python", **kwargs): return executor.run(code, language, **kwargs)
def run_code(code, language="python", **kwargs): return code_execute(code, language, **kwargs)
