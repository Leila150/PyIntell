...
class Model:
    ...
    def __init__(self, vocab, reverse_vocab, dataset, parameters, focus, dtype, settings):
        ...
        if not isinstance(reverse_vocab, dict):
            raise TypeError("reverse_vocab must be a dictionary")
        self.vocab = vocab
        self.reverse_vocab = reverse_vocab
        self.dataset = dataset
        self.requested_parameters = int(parameters)
        if self.requested_parameters < 0:
        ...
        if min(self.layers, self.heads, self.embedding_size, self.context_length, self.hidden_size) < 1:
            raise ValueError("layers, heads, embedding_size, context_length, and hidden_size must be positive")
    ...
    def _named_parameters(self):
        ...
        for i, block in enumerate(self.blocks):
            for name, value in block.items(): params[f"blocks.{i}.{name}"] = value
        return params

    def named_parameters(self): return self._named_parameters()
    def parameter_count(self): return int(sum(value.size for value in self._named_parameters().values()))
    ...
    def run(self):
        """Make this model the active model and return it."""
        from .serialization import set_current_model
        return set_current_model(self)

    def inference(self):
        """Create a reusable inference session for this model."""
        from .inference import InferenceSession
        return InferenceSession(self)

    def get_focus(self): return list(self.focus)

    def set_focus(self, focus):
        config = build_focus_config(focus)
        self.focus = tuple(config["focuses"]); self.focus_config = config
        self.focus_priorities = dict(config["priorities"]); self.settings["focus_config"] = config
        return self.get_focus()

    def focus_scores(self): return dict(self.focus_priorities)

    def focus_info(self):
        ranked = sorted(self.focus_priorities.items(), key=lambda item: (-item[1], item[0]))
        return {"focuses": self.get_focus(), "priorities": self.focus_scores(),
    ...
    def forward(self, token_ids):
        ...
        if len(ids) == 0: raise ValueError("token_ids must not be empty")
        if len(ids) > self.context_length: raise ValueError("sequence exceeds model context length")
        if ids.min() < 0 or ids.max() >= len(self.vocab): raise ValueError("token ID outside vocabulary")
    ...
    def logits(self, token_ids): return self.forward(token_ids) @ self.output_weights + self.output_bias
    ...
    def train(self, dataset=None, epochs=1, learning_rate=None, optimizer=None, **kwargs):
        ...
        for _ in range(epochs):
            ...
            for inputs, target in examples:
                ...
                gradients["output"] += np.outer(hidden, probabilities); gradients["output_bias"] += probabilities
            gradients = {k: v / len(examples) for k, v in gradients.items()}
            self.optimizer.step(gradients); self.training_steps += 1
        ...
        record = {"loss": history[-1], "loss_history": history, "epochs": epochs, "samples": len(examples),
                  ...
                  "loss_decreased": after["loss"] < before["loss"], "accuracy_improved": after["accuracy"] > before["accuracy"]}
        self.training_history.append(record); return record

    def generate(self, prompt, max_tokens=50, temperature=1.0, top_k=None, **kwargs):
        if isinstance(max_tokens, bool) or not isinstance(max_tokens, (int, np.integer)): raise TypeError("max_tokens must be an integer")
        ...
        if isinstance(prompt, str):
            ...
            if any(token_id is None for token_id in ids): raise ValueError("prompt contains unknown tokens and vocabulary has no '<unk>' token")
        else: ids = list(prompt)
        if not ids: raise ValueError("prompt must contain at least one known token")
        if any(int(i) < 0 or int(i) >= len(self.vocab) for i in ids): raise ValueError("prompt contains an invalid token ID")
        if temperature <= 0: raise ValueError("temperature must be > 0")
        for _ in range(max_tokens):
            context = ids[-self.context_length:]
            ids.append(int(sample(self.logits(context)[-1], temperature=temperature, top_k=top_k)))
    ...
    def summary(self):
        ...
        return {"model_name": self.model_name, "platform": self.platform, "device": self.device,
                "parameters": actual, "requested_parameters": self.requested_parameters, "focus": list(self.focus),
                "focus_scores": self.focus_scores(), "dtype": self.requested_dtype, "layers": self.layers,
                "heads": self.heads, "embedding_size": self.embedding_size, "hidden_size": self.hidden_size,
                "context_length": self.context_length, "vocab_size": len(self.vocab), "training_steps": self.training_steps,
                "training_history_length": len(self.training_history), "settings": dict(self.settings)}
