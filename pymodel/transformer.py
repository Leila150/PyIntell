...
def decoder(x, context=None, layers=4, heads=8, hidden_size=None):
    """Run a causal decoder stack, optionally attending to encoder context."""
    for _ in range(layers):
        normalized = layer_norm(x)
        x = x + multihead_attention(normalized, heads=heads, mask=causal_mask(x.shape[-2]))
